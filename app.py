...
def main():
    ...
    if file and number1 :
        if file.name[-3:] == 'csv':
            ...
            st.dataframe(df.head(10), use_container_width=st.session_state.use_container_width)
            st.info(score)
            ...
            df2=txt_input(df)
            st.checkbox("Use container width", value=False, key="use_container_width")
            st.dataframe(df2.head(10), use_container_width=st.session_state.use_container_width)
